...
class sentinelSLC(object):
    ...
    def get_param(self):
        ...
        elem = root.find('.//metadataObject[@ID="processing"]')#Encuentra la version de procesamiento
...
def get_information_safe(dir_slc):
    ...
    zips = sorted(glob.glob(os.path.join(dir_slc, 'S1*_IW_SLC_*.zip')), key=lambda x: x.split('_')[-5], reverse=False)
    nzips = len(zips)
    ...
    for i in range(nzips):
        ...
        safeObj.get_param()
        safe_list.append({'file':safeObj.safe_file,
                          'IPF Version':safeObj.proc_version})
    return safe_list
...
def get_information_orbit(dir_orbit):
    ...
    for orbit_i in orbits:
        ...
        orbit_files_out.append({'file':orbit_i,
    ...
    if list_scenes_out!=[]:
        ...
        with open(report_out,'w') as fl:
            for item in list_scenes_out:
                fl.write('%s\n'%item)
                fl.close()
        print('A file to exclude dates by IPF versions was created \n exclude_by_IPF.txt')
...
def cmdLineParse():
    ...
    parser = argparse.ArgumentParser( description='Report bad orbits and scenes with IPF versions < 2.6')
    parser.add_argument('-dirSlc', dest='dirSlc', type=str, required=True,
            help = 'directory with the "S1*_IW_SLC_*.zip" files')
    parser.add_argument('-dirOrb', dest='dirOrb', type=str, required=True,
